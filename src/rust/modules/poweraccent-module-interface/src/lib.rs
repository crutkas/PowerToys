...
static MODULE_WIDE: std::sync::LazyLock<&'static [u16]> = std::sync::LazyLock::new(|| {
    Box::leak(to_wide("QuickAccent").into_boxed_slice())
});
...
impl Module {
    pub fn new() -> Self {
        Self {
            ...
            process_handle: None,
            terminate_event: create_event("Local\\PowerToysPowerAccentExitEvent-53e93389-d19a-4fbb-9b36-1981c8965e17"),
        }
    }
    ...
    fn launch_process(&mut self) {
        ...
        let exe = to_wide("PowerToys.PowerAccent.exe");
...
mod tests {
    ...
    fn test_name_and_key() {
        ...
        assert_eq!(name, "QuickAccent");
        assert_eq!(key, "QuickAccent");
    }
