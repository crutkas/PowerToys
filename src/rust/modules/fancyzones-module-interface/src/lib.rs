...
static MODULE_WIDE: std::sync::LazyLock<&'static [u16]> = std::sync::LazyLock::new(|| {
    Box::leak(to_wide("FancyZones").into_boxed_slice())
});
...
impl Module {
    pub fn new() -> Self {
        Self {
            ...
            process_handle: None,
            terminate_event: create_event("Local\\PowerToys-FZE-ExitEvent-ca8c73de-a52c-4274-b691-46e9592d3b43"),
        }
    }
    ...
    fn launch_process(&mut self) {
        ...
        let exe = to_wide("PowerToys.FancyZones.exe");
...
mod tests {
    ...
    fn test_name_and_key() {
        ...
        assert_eq!(name, "FancyZones");
        assert_eq!(key, "FancyZones");
    }
