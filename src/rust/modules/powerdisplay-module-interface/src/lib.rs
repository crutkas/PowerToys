...
static MODULE_WIDE: std::sync::LazyLock<&'static [u16]> = std::sync::LazyLock::new(|| {
    Box::leak(to_wide("PowerDisplay").into_boxed_slice())
});
...
impl Module {
    pub fn new() -> Self {
        Self {
            ...
            process_handle: None,
            terminate_event: create_event("Local\\PowerToysPowerDisplay-TerminateEvent-7b9c2e1f-8a5d-4c3e-9f6b-2a1d8c5e3b7a"),
        }
    }
    ...
    fn launch_process(&mut self) {
        ...
        let exe = to_wide("WinUI3Apps\\PowerToys.PowerDisplay.exe");
...
mod tests {
    ...
    fn test_name_and_key() {
        ...
        assert_eq!(name, "PowerDisplay");
        assert_eq!(key, "PowerDisplay");
    }
