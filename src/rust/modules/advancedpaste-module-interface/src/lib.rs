...
static MODULE_WIDE: std::sync::LazyLock<&'static [u16]> = std::sync::LazyLock::new(|| {
    Box::leak(to_wide("AdvancedPaste").into_boxed_slice())
});
...
impl Module {
    ...
    fn launch_process(&mut self) {
        ...
        let exe = to_wide("WinUI3Apps\\PowerToys.AdvancedPaste.exe");
...
mod tests {
    ...
    fn test_name_and_key() {
        ...
        assert_eq!(name, "AdvancedPaste");
        assert_eq!(key, "AdvancedPaste");
    }
