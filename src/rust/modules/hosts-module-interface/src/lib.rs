//! Hosts Module Interface — Rust implementation
//!
//! This is the PowerToys module DLL for Hosts File Editor.
//! It launches `PowerToys.Hosts.exe` when enabled.
//!
...
static MODULE_WIDE: std::sync::LazyLock<&'static [u16]> = std::sync::LazyLock::new(|| {
    Box::leak(to_wide("Hosts").into_boxed_slice())
});
...
impl Module {
    ...
    fn launch_process(&mut self) {
        ...
        let exe = to_wide("WinUI3Apps\\PowerToys.Hosts.exe");
...
impl PowerToyModule for Module {
    ...
    fn gpo_policy_enabled_configuration(&self) -> GpoRuleConfigured {
        check_gpo("ConfigureEnabledUtilityHostsFileEditor")
    }
}
