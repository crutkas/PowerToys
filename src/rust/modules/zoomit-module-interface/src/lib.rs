...
static MODULE_WIDE: std::sync::LazyLock<&'static [u16]> = std::sync::LazyLock::new(|| {
    Box::leak(to_wide("ZoomIt").into_boxed_slice())
});
...
impl Module {
    pub fn new() -> Self {
        Self {
            ...
            process_handle: None,
            terminate_event: create_event("Local\\PowerToysZoomIt-ExitEvent-36641ce6-df02-4eac-abea-a3fbf9138220"),
        }
    }
    ...
    fn launch_process(&mut self) {
        ...
        let exe = to_wide("PowerToys.ZoomIt.exe");
...
mod tests {
    ...
    fn test_name_and_key() {
        ...
        assert_eq!(name, "ZoomIt");
        assert_eq!(key, "ZoomIt");
    }
