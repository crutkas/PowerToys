...
static MODULE_WIDE: std::sync::LazyLock<&'static [u16]> = std::sync::LazyLock::new(|| {
    Box::leak(to_wide("MouseWithoutBorders").into_boxed_slice())
});
...
impl Module {
    ...
    fn launch_process(&mut self) {
        ...
        let exe = to_wide("PowerToys.MouseWithoutBorders.exe");
...
impl PowerToyModule for Module {
    ...
    fn gpo_policy_enabled_configuration(&self) -> GpoRuleConfigured {
        check_gpo("EnableMouseWithoutBorders")
    }
}
...
mod tests {
    ...
    fn test_name_and_key() {
        ...
        assert_eq!(name, "MouseWithoutBorders");
        assert_eq!(key, "MouseWithoutBorders");
    }
